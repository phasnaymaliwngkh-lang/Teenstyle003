...
**ล่าสุด: STEP 34 เสร็จ — Performance (วัดกับข้อมูลปริมาณจริง แล้วแก้เฉพาะที่วัดได้)**
เขียน [`scripts/audit-performance.mjs`](../scripts/audit-performance.mjs) ที่สร้างฐานข้อมูล
ชั่วคราวแยก ลง migration ชุดเดียวกับของจริง เติมข้อมูลปริมาณจริง (**สินค้า 5,000 · ตัวเลือก 20,000 ·
ผู้ใช้ 20,000 · คำสั่งซื้อ 60,000 · รายการในบิล 120,000 · ประวัติหลังบ้าน 150,000**) แล้ว
**เปิด backend ตัวจริง** ชี้ไปฐานข้อมูลนั้น ยิงทุก endpoint ที่หนัก เทียบกับงบเวลาต่อเส้นทาง
แล้วลบฐานข้อมูลทิ้ง — **ไม่แตะฐานข้อมูลของแอปเลย** · ทำไมต้องทำถึงขั้นนั้น: ฐานข้อมูล dev
มีสินค้า 12 ชิ้นและคำสั่งซื้อ 0 ใบ ทุกคิวรีจึงเร็วเท่ากันหมด และ planner เลือก seq scan ทุกครั้ง
→ **ปัญหาทุกข้อด้านล่างมองไม่เห็นเลยตอน dev** ·
**ของที่เจอ:** ① หน้า `/shop?sort=bestselling` ซึ่งเป็น**หน้าสาธารณะ** ใช้ **552ms** เพราะเรียงด้วย
subquery ที่อ้างแถวของตัวเอง → PostgreSQL รันซ้ำทีละสินค้า (5,000 รอบ) ② แพตเทิร์นเดียวกันอีก 5 ที่
(ตัวกรอง "พร้อมส่ง" · ของหมด/สต็อกต่ำบน dashboard · รายการสินค้าหลังบ้าน · จำนวนข้างตัวกรอง ·
เรียงตามความนิยม) ③ ค้นหาที่ `/admin/orders` ใช้ `OR` **ข้ามสองตาราง** ซึ่ง **index ช่วยไม่ได้เลย**
(ใส่ trigram แล้ววัดได้ 267→245ms เพราะแผนคิวรีไม่เปลี่ยน) ④ **ไม่มี index ของ `paidAt`**
ทั้งที่รายงานยอดขายทุกแบบตัดรอบด้วยคอลัมน์นี้ตามกฎ STEP 26 ⑤ `/api/admin/stock-alerts` ส่ง
**1.1 MB ต่อคำขอ** เพราะคืนทุกแถวที่ตกเกณฑ์ (3,659 รายการ) ·
**ผลหลังแก้:** bestselling 552→121ms · ค้นหาออเดอร์ 313→37ms (ชั้น SQL 267→4ms) ·
ค้นด้วยชื่อลูกค้าไทย 411→25ms · รายการสินค้าหลังบ้าน 165→77ms · รายงานรายวัน 42→5.8ms ·
ค้นหาลูกค้า 68→2.3ms · stock-alerts 1,168→64 KB · **30 เส้นทางอยู่ในงบทั้งหมด** ·
**ยึดหลัก "ไม่เพิ่ม index เพราะน่าจะช่วย"** — วัด A/B ทีละตัว (ลบ → วัด → สร้าง → วัด)
แล้วเพิ่มเฉพาะ 5 ตัวที่เห็นผลต่างจริง · trigram ของ `Product.name` **ไม่เพิ่ม** เพราะที่ 5,000 สินค้า
planner เลือก seq scan เองอยู่แล้ว (วัดได้ 0.9× คือช้าลงเล็กน้อย) ·
**ปิดหนี้เก่าด้วยการวัด:** STEP 4 จดไว้ว่า "navbar อ่าน session ทำให้ทุกหน้าเป็น dynamic
ให้แก้ตอน STEP 33/34" — วัดแล้วพบว่าใช้เวลาแค่ ~1.2ms และ **แก้ไม่ได้อยู่ดี** เพราะ CSP แบบ nonce
ของ STEP 28 ทำให้ prerender HTML ล่วงหน้าไม่ได้ · **Redis ยังไม่มีและไม่แกล้งว่ามี** —
เขียนไว้ตรง ๆ ว่า rate limit นับแยกต่อ process (limit จริง = ค่าที่ตั้ง × จำนวน instance)
พร้อมคำเตือนใน log ตอนบูต production · รายละเอียดทั้งหมดใน [10-performance.md](10-performance.md)

**STEP 33 — SEO (ทำให้ทุกหน้าบอก Google ตรงกับความจริง และตรวจได้ด้วยเครื่อง)**
...
|   34 | Performance (cache, index, Redis) |  ✅   | วัดกับข้อมูลปริมาณจริงด้วย [`scripts/audit-performance.mjs`](../scripts/audit-performance.mjs) (30 เส้นทางอยู่ในงบ) · ถอด correlated subquery 6 ที่ (bestselling 552→121ms) · ค้นหาออเดอร์ OR ข้ามตาราง → UNION + trigram (313→37ms) · เพิ่ม index 5 ตัวที่วัดแล้วช่วยจริง · จำกัดขนาด response ของ stock-alerts (1.1MB→64KB) · **Redis ยังไม่มี** (rate limit นับแยกต่อ process) — [10-performance.md](10-performance.md) |
