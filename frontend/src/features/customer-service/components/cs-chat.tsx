...
export function CsChat() {
  ...
  return (
    <div className="flex h-[750px] w-full flex-col rounded-3xl border border-line bg-white shadow-lift overflow-hidden">
      ...
      {status === "ESCALATED" && (
        <div className="flex items-center gap-2 border-b border-warning/30 bg-warning/5 px-5 py-2.5 text-xs text-warning">
      ...
      <div
        ...
        className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-lilac-50/40"
      >
        {initialLoading ? (
          ...
          messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                >
                  {!isUser && (
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      {isAgent && (
                        <span className="rounded bg-lilac px-1.5 py-px text-[9px] font-semibold text-brand-dark">
                          Staff
                        </span>
                      )}
                    </div>
                  ...
                  <div
                    className={`rounded-2xl p-3.5 text-xs sm:text-sm leading-relaxed whitespace-pre-line shadow-xs ${
                      isUser
                        ...
                        : isAgent
                          ? "rounded-tl-xs border border-brand-soft/40 bg-lilac-50/70 text-ink font-normal"
                ...
                {isUser && (
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-lilac text-ink-soft text-xs">
      ...
      {error && (
        <div
          role="alert"
          className="border-t border-danger/25 bg-danger/5 px-4 py-2 text-xs text-danger flex items-center justify-between"
        >
          <span>{error}</span>
          <button
            type="button"
