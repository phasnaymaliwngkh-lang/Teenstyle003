 ...
 *
 * cache นี้ทำงานจริงและวัดผลได้: ที่ข้อมูลระดับ 5,000 สินค้า / 60,000 คำสั่งซื้อ
 * หน้า `/shop` ที่ cache อุ่นตอบใน ~30ms แทนที่จะรอ API 30–150ms ทุกครั้ง
 *
 * ⚠️ **`tags` ยังไม่มีใครเรียก `revalidateTag()` — ตรวจแล้วตอน STEP 34 ว่าไม่มีเลยทั้งโปรเจกต์**
 *    แปลว่าตอนนี้ cache หมดอายุด้วย **เวลาเท่านั้น** (60 วินาที) แก้ราคาที่หลังบ้านแล้ว
 *    หน้าร้านจึงช้าได้ถึง 60 วินาที ซึ่งเป็นพฤติกรรมที่โปรเจกต์รับไว้ตั้งแต่ STEP 5/23
 *    (ดูหัวข้อ stale-while-revalidate ใน CLAUDE.md)
 *
 *    เก็บ `tags` ไว้เพราะมันคือ **จุดเดียว** ที่ต้องแก้เมื่อทำการล้าง cache แบบเจาะจง
 *    (backend ยิง route handler ของ Next → `revalidateTag('products')`) และห้ามยืด
 *    `REVALIDATE_SECONDS` ให้นานกว่านี้ก่อนที่การล้าง cache จะใช้งานได้จริง
 *    ไม่งั้นข้อมูลราคาจะค้างนานขึ้นตามโดยไม่มีทางล้าง
 */
